...
ALLOWED_GRANULARITY = {"hour": 3600.0, "day": 86400.0, "week": 604800.0}
...
bucket_seconds = ALLOWED_GRANULARITY[GRANULARITY]

sql = f"""
select
    date_trunc('{GRANULARITY}', block_time) as time,
    count(*) as tx_count,
    count(*) / {bucket_seconds} as avg_tps,
...
fig = px.line({{__df_name}}, x="time", y=["dau", "avg_tps"])
fig.show()
