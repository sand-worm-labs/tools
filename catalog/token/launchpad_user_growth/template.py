...
# EVM generalization: launchpad "users" reinterpreted as wallets whose transactions
# triggered an EVM factory contract to deploy a token (tx sender behind the call).
import re
...
with user_activity as (
    select date_trunc('day', c.block_time) as day, t."from" as wallet
...
daily as (
    select
        ...
        count(distinct case when f.first_day = a.day then a.wallet end) as new_users
